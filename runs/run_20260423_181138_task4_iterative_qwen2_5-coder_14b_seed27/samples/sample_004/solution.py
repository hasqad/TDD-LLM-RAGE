def sort_library_books(books):
    ...
    sorted_books = sorted(books, key=lambda x: (x['due_date'], -x['priority'], genre_popularity.get(x['genre'], 0), x['id']))

    # Step 5: Group overlapping reservations together
    result = []
    current_group = []
    for book in sorted_books:
        if not current_group or any(r1[0] < r2[1] and r1[1] > r2[0] for r1 in book['reservations'] for r2 in [r for b in current_group for r in b['reservations']]):
            if current_group:
